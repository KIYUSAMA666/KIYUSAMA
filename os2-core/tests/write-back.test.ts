...
function input(revision=5){return {current:current(revision),handoff:handoff(revision),handoffDecision:{status:"READY"},result:result(revision),resultDecision:{status:"ACCEPTED",outcome:"SUCCEEDED"},consumedResultIds:new Set(),consumedHandoffIds:new Set()};}
