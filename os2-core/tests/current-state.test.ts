...
import { assertSnapshotInvariant, selectCurrentSnapshot } from "../src/current-state.js";
...
test("5 different scope conflicts", () => { const b=snapshot(2); b.identity.scope="OTHER"; assert.equal(selectCurrentSnapshot(snapshot(1),b).status,"STATE_CONFLICT"); });
