...
function handoff(){return{handoffId:"HO-1",traceId:"TRACE-1",actionId:"NA-1",sourceStateId:"CS-1",sourceStateRevision:1,requiredRole:"EXECUTION_AUTHORITY",executorAuthorityId:"EXECUTOR-1",capabilityId:"CAP-A",implementationId:"IMPL-1",issuedAt:"2026-09-10T16:00:00+09:00",expiresAt:"2026-09-10T17:00:00+09:00",evidenceRefs:[{id:"REF-HANDOFF-1",expectedVersion:"1",path:"evidence/REF-HANDOFF-1"}],resultEvidencePolicy:{requiredRefs:[ref()],verifierId:"KIRA-1",evidenceSource:"KIRA-1"}}}
...
test("21 executor substitution holds",()=>{const i=input();i.evidence.executorId="ATTACKER";assert.deepEqual(evaluateExecutionResultEvidence(i),{status:"HOLD",reason:"EXECUTOR_AUTHORITY_MISMATCH"})});
test("22 handoff executor authority drift holds",()=>{const i=input();i.handoff.executorAuthorityId="EXECUTOR-2";assert.deepEqual(evaluateExecutionResultEvidence(i),{status:"HOLD",reason:"EXECUTOR_AUTHORITY_MISMATCH"})});
test("23 CURRENT authority drift holds",()=>{const i=input();i.snapshot.activeRolesAndAuthority.EXECUTION_AUTHORITY="EXECUTOR-2";assert.deepEqual(evaluateExecutionResultEvidence(i),{status:"HOLD",reason:"EXECUTOR_AUTHORITY_MISMATCH"})});
