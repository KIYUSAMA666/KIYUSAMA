...
function provider(revision=7){return{readCurrentState:()=>snapshot(revision)}}
function base(overrides={}){const h=verifiedHandoff();return{handoffRequest:h.request,handoffReceipt:h.receipt,currentStateProvider:provider(),expectedWorker:worker,intent:intent(),now:NOW,...overrides}}

test("1 exact verified handoff plus freshly loaded CURRENT issues a narrow permit",()=>{const d=issueSideEffectPermit(base());assert.equal(d.status,"PERMIT");assert.equal(d.permit.target,"slack:C0BLC7U76FR");assert.equal(d.permit.operation,"chat.postMessage");assert.equal(d.permit.runtimeScope,SIDE_EFFECT_PERMIT_RUNTIME_SCOPE)});
...
test("3 live CURRENT state id drift is rejected",()=>{const b=base();b.currentStateProvider={readCurrentState:()=>({...snapshot(),identity:{...snapshot().identity,stateId:"CS-2"}})};assert.deepEqual(issueSideEffectPermit(b),{status:"HOLD",reason:"CURRENT_BINDING_MISMATCH"})});
test("4 live CURRENT revision drift is rejected",()=>assert.deepEqual(issueSideEffectPermit(base({currentStateProvider:provider(8)})),{status:"HOLD",reason:"CURRENT_BINDING_MISMATCH"}));
...
test("16 KIRA TOCTOU reproduction is closed by mandatory fresh CURRENT provider",()=>{const b=base({currentStateProvider:provider(9)});assert.deepEqual(issueSideEffectPermit(b),{status:"HOLD",reason:"CURRENT_BINDING_MISMATCH"})});
test("17 CURRENT provider failure fails closed",()=>{const b=base({currentStateProvider:{readCurrentState(){throw new Error("backend down")}}});assert.deepEqual(issueSideEffectPermit(b),{status:"HOLD",reason:"CURRENT_PROVIDER_FAILURE"})});
test("18 permit is explicitly in-process only and serialization invalidates authenticity",()=>{const b=base();const d=issueSideEffectPermit(b);assert.equal(d.status,"PERMIT");const rehydrated=JSON.parse(JSON.stringify(d.permit));assert.equal(rehydrated.runtimeScope,"IN_PROCESS_ONLY");assert.equal(isVerifiedSideEffectPermit(rehydrated,b.intent,NOW),false)});
