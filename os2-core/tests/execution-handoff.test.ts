...
function input(){return{snapshot:snapshot(),actionEvidenceRequirement:{actionId:"NA-1",requiredRefs:[ref()],requireIndependentLane:false},capabilitySlot:capabilitySlot(),gateDecision:{status:"ALLOW",actionId:"NA-1",stateId:"CS-1",stateRevision:1,role:"EXECUTION_AUTHORITY",actorAuthorityId:"EXECUTOR-1"}}}
function request(){return{handoffId:"HO-1",traceId:"TRACE-1",actionId:"NA-1",sourceStateId:"CS-1",sourceStateRevision:1,requiredRole:"EXECUTION_AUTHORITY",executorAuthorityId:"EXECUTOR-1",capabilityId:"CAP-A",implementationId:"IMPL-1",issuedAt:"2026-09-10T16:00:00+09:00",expiresAt:"2026-09-10T17:00:00+09:00",evidenceRefs:[ref()],resultEvidencePolicy:{requiredRefs:[ref("REF-RESULT-1","1","evidence/REF-RESULT-1")],verifierId:"KIRA-1",evidenceSource:"KIRA-1"}}}
...
test("22 handoff executor substitution holds",()=>{const r=request();r.executorAuthorityId="ATTACKER";assert.deepEqual(evaluateExecutionHandoff(input(),r,NOW),{status:"HOLD",reason:"AUTHORITY_BINDING_MISMATCH"})});
test("23 handoff role substitution holds",()=>{const r=request();r.requiredRole="OTHER_ROLE";assert.deepEqual(evaluateExecutionHandoff(input(),r,NOW),{status:"HOLD",reason:"AUTHORITY_BINDING_MISMATCH"})});
test("24 CURRENT authority drift holds",()=>{const i=input();i.snapshot.activeRolesAndAuthority.EXECUTION_AUTHORITY="EXECUTOR-2";assert.deepEqual(evaluateExecutionHandoff(i,request(),NOW),{status:"HOLD",reason:"AUTHORITY_BINDING_MISMATCH"})});
